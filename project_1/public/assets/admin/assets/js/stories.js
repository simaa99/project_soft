var stories_tbl = $("#stories_tbl");
$(document).ready(function () {


    if ($("#stories_tbl").length) {

        stories_tbl.on('preXhr.dt', function (e, settings, data) {
            // data.username = $('#username_c').val();
        }).dataTable({
            "processing": true,
            "serverSide": true,
            "ajax": {
                url: baseURL + "/web/story-data",
                "dataSrc": function (json) {
                    //Make your callback here.
                    if (json.status !== undefined && !json.status) {
                        $('#stories_tbl_processing').hide();
                        location.reload();
                        //
                    } else
                        return json.data;
