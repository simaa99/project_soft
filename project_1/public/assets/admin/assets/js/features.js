var features_tbl = $("#features_tbl");
$(document).ready(function () {


    if ($("#features_tbl").length) {

        features_tbl.on('preXhr.dt', function (e, settings, data) {
            // data.username = $('#username_c').val();
        }).dataTable({
            "processing": true,
            "serverSide": true,
            "ajax": {
                url: baseURL + "/web/feature-data",
                "dataSrc": function (json) {
                    //Make your callback here.
                    if (json.status !== undefined && !json.status) {
                        $('#features_tbl_processing').hide();
                        location.reload();
                        //
                    } else
                        return json.data;
